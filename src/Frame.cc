     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    TrackingStats::getInstance().stereoMatch_time.emplace_back(mnId, mTimeStereoMatch_);
#endif
#ifdef REGISTER_TIMES
...
void Frame::ComputeStereoMatchesGPU()
{
    const int thOrbDist = (ORBmatcher::TH_HIGH+ORBmatcher::TH_LOW)/2;
    ...
    //Assign keypoints to row table
    vector<vector<int> > vRowIndices(nRows,vector<int>());

    for(int i=0; i<nRows; i++)
        vRowIndices[i].reserve(MAX_FEATURES_IN_ROW_SLIDING_WINDOW);
...
void Frame::ComputeStereoFishEyeMatchesGPU() {
    mvLeftToRightMatch = vector<int>(Nleft,-1);
    mvRightToLeftMatch = vector<int>(Nright,-1);
    mvDepth = vector<float>(Nleft,-1.0f);
    mvuRight = vector<float>(Nleft,-1);
    ...
    for(int i = 0; i < mvKeys.size(); i++) {
        ...
        Eigen::Vector3f p3D;
        descMatches++;
        float sigma1 = mvLevelSigma2[mvKeys[i].octave], sigma2 = mvLevelSigma2[mvKeysRight[matches[i]].octave];
        float depth = static_cast<KannalaBrandt8*>(mpCamera)->TriangulateMatches(mpCamera2,mvKeys[i],mvKeysRight[matches[i]],mRlr,mtlr,sigma1,sigma2,p3D);
        if(depth > 0.0001f){
            mvLeftToRightMatch[i] = matches[i];
            mvRightToLeftMatch[matches[i]] = i;
            mvStereo3Dpoints[i] = p3D;
            mvDepth[i] = depth;
            nMatches++;
        }
    }
